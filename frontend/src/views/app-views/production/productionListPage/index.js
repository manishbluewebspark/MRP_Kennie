...
//     const formatted = formatProjectType(projectType); // "Box Build" | "Cable Harness"
...
//   const getStageState = (record) => {
//     const qtyRequired = Number(record.quantity || 0);
...
//       const totalQty = history
//         .filter((p) => p.process === stage.key)
//         .reduce((sum, p) => sum + (Number(p.qty) || 0), 0);

//       const hasAnyActivity = history.some((p) => p.process === stage.key);
//       const isCompleted = qtyRequired > 0 && totalQty >= qtyRequired;
...
//     // 🔍 Active stage logic:
//     // 1) first stage which is NOT completed
//     // 2) if all completed → last one active (no blink if you don't want)
//     let activeKey = null;

//     const firstNotCompleted = stages.find((s) => !s.isCompleted);
//     if (firstNotCompleted) {
//       activeKey = firstNotCompleted.key;
//     } else if (stages.length) {
//       activeKey = stages[stages.length - 1].key;
//     }
...
//     const { stages } = getStageState(record);
//     if (!stages.length) return 0;

//     // simple: average completion ratio per stage
//     const qtyRequired = Number(record.quantity || 0) || 1;

//     const totalRatio =
//       stages.reduce((sum, s) => {
//         const ratio = Math.min(1, s.totalQty / qtyRequired);
//         return sum + ratio;
//       }, 0) / stages.length;
...
//         {(() => {
//           const { stages, activeKey } = getStageState(record);
...
//                       title={
//                         `Stage: ${stage.label} | ` +
//                         `Done: ${stage.totalQty}/${record.quantity}`
//                       }
//                     >
//                       <Card
//                         size="small"
...
//                           // 🔥 INLINE BLINK ANIMATION HERE ↓↓↓
//                           animation: isActive && !isCompleted ? "blinkAnim 1s infinite" : "none",
//                         }}
//                         onClick={() => {
